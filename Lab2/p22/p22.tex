...
\item Cuando se hace login (vía consola ssh, etc.) el .bash\_profile se ejeucta para configurar el shell antes de que salga el prompt. Pero si ejecutas un xterm dentro de Gnome o KDE, entonces es el .bashrc el que se ejecuta (aunque también lo hace cuando se ejeucta una instancia de /bin/bash).
\item A efectos prácticos, configurar el .bash\_profile para configuraciones de nuestro terminal.
...
\begin{definición}
  \textbf{Shell Script, guión shell: }Es un fichero de texto ejecutable que contiene órdenes ejecutables por el shell.
\end{definición}
...
\item HOME: directorio de trabajo del usuario actual. (cd \$HOME ó cd $\sim$).
...
\item \$-: las opciones actuales suministradas para esta invocación del shell. (se pueden consultar mediante \textbf{ info bash --index-search=set})
\item \$*: todos los argumentos del shell comenzando por el \$1. Esto es, “\$*” es equivalente a “\$1 \$2. . . .”, donde los parámetros se separan por espacios. 
...
\subsection*{Órdenes internas de Bash}
...
\begin{itemize}
\item echo: envía una cadena a la salida estándar, normalmente la consola o una tubería. Por ejemplo:\\ 
\textbf{echo El valor de la variable es \$auxvar}

\item read: lee una cadena de la entrada estándar y la asigna a una variable, permitiendo obtener entrada de datos por teclado en la ejecución de un guión shell:
...
\item let arg [arg]: cada arg es una expresión aritmética a ser evaluada:
\begin{lstlisting}[language=bash]
$ b=11
$ let a=$b+7
$ echo $a
...
\textbf{test} es una orden que permite evaluar si una expresión es verdadera o falsa. Los tests no sólo operan sobre los valores de las variables, también permiten conocer, por ejemplo, las propiedades de un fichero.\\
...
[ expresión ]
\end{lstlisting}
\emph{Atención: }Los espacios en blanco entre la expresión y los corchetes son necesarios. La expresión puede incluir operadores de comparación como los siguientes:
...
\begin{itemize}
\item Para caracteres alfabéticos o cadenas:
...
\item En expresión se pueden incluir operaciones con ficheros, entre otras:
\begin{figure}[ht!]
\centering
...
\item script2.sh: muestra su número PID y después llama a un programa llamado \textbf{num} a través de del punto (. num). Cuando num termina su ejecución, la orden (.) devuelve el control al programa que lo llamó, el cual muestra el mensaje. (Nota: el fichero num lleva un sólo \textbf{echo "Soy num"}).
...
\item script8.sh: Escribir un guión shell que liste los directorios existentes en el directorio actual.

\item script9.sh: script que pida por teclado un número PID de proceso y lo elimine.

\item script10.sh: Escribir un guión shell llamado ver que para cada argumento que reciba realice una de las siguientes operaciones:
\begin{itemize}
